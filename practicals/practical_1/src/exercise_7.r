...
calculate_sd_for_sample_size <- function(
    ...
    lambda,
    m) {
    means_for_n_samples <- replicate(m, calculate_sd_n_times(k = k, lambda = lambda, num_samples = num_samples))
...
n_samp <- seq(5, 100, 5)
m <- 1000
lapply(n_samp, function(x) calculate_sd_for_sample_size(num_samples = x, k = k, lambda = lambda, m = m))
